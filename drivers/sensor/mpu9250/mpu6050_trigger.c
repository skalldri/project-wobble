...
int mpu6050_init_interrupt(struct device *dev)
{
	...
	if (i2c_reg_write_byte(drv_data->i2c, CONFIG_MPU6050_I2C_ADDR,
			       MPU6050_REG_INT_EN, MPU6050_DRDY_EN) < 0) { // don't really want the data-ready pin ( want high interupt)
		SYS_LOG_ERR("Failed to enable data ready interrupt.");
		return -EIO;
	}

#if defined(CONFIG_MPU6050_TRIGGER_OWN_THREAD) // data collection running on it's own thread ( this is what we want)
